...
import ru.shanalotte.bankbarrel.rest.infomodule.service.caching.RedisCachingManager;
...
public class AccountTypesController {
  ...
  @Autowired
  public AccountTypesController(CachingManager cachingManager, AccountTypesReader accountTypesReader) {
  ...
  public ResponseEntity<List<CodeAndValuePair>> getAdditionalAccountTypes(
    ...
    if (cachingManager.hasKey(cacheKey)) {
      return new ResponseEntity<>(convertCachedValuesToResponse(cachedAdditionalAccountTypes(code)), HttpStatus.OK);
    }
  ...
  private Map<String, String> convertResponseToCachedValues(List<CodeAndValuePair> codeAndValuePairs) {
  ...
  private List<CodeAndValuePair> cachedAccountTypes() {
    Map<String, String> cachedValues = (Map<String, String>) cachingManager.getValueFromCache(ACCOUNT_CACHE_KEY);
